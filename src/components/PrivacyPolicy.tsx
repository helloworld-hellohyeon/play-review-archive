...
export function PrivacyPolicy() {
  return (
    <Wrap>
      ...
      <p>
        익스텐션은 사용자가 열람 중인 Twitter/X 스레드 페이지의 DOM을 읽어 트윗 텍스트와 이미지
        URL을 추출합니다. 추출된 데이터는 사용자 기기에 ZIP 파일로 저장되며, 어떠한 외부 서버로도
        전송되지 않습니다.
      </p>

      <Section>외부 서버</Section>
      <p>
        이 익스텐션이 운영하는 서버는 없습니다. 분석 도구, 원격 로깅, 광고 네트워크를 사용하지
        않습니다.
      </p>
      ...
      <p>
        {`The extension reads the DOM of the Twitter/X thread page currently open in the browser to
        extract tweet text and image URLs. All extracted data is saved as a ZIP file on the user's
        device only and is never transmitted to any external server.`}
      </p>

      <Section>External Servers</Section>
      <p>
        This extension operates no servers. No analytics tools, remote logging, or ad networks are
        used.
      </p>
